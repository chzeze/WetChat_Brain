...
//根据题目返回,进行答案搜索
func handleQuestionResp(bs []byte) (bsNew []byte,ans int) {
	bsNew = bs
	question := &Question{}
	...
	if question.CalData.TrueAnswer != "" {
		...
		for i, option := range respQuestion.Data.Options {
			str1 := strconv.Itoa(cnt)
			if option == question.CalData.TrueAnswer {
				respQuestion.Data.Options[i] = str1+option + "[标准答案]"
				ans=cnt
				break
			}
			cnt=cnt+1
		...
		for i, option := range respQuestion.Data.Options {
			str1 := strconv.Itoa(cnt)
			if ret[option] > 0 {
				respQuestion.Data.Options[i] = str1+option + "[" + strconv.Itoa(ret[option]) + "]"
				if ret[option] > maxScore{
					ans=cnt	
					maxScore=ret[option]
				}
